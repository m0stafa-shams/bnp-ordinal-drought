...
#####################################################
######## BP model for ordinal drought data #########
#####################################################
...
mod_constants=list(Tobs=Tobs, bp=bp, cut=c(0,1,2,3,4),numB=numB)
...
model_code=nimbleCode({
  ...
  for(t in 1:Tobs){
    mu[t] <- beta0 + inprod(X[t,2:bp],beta[2:bp]) + inprod(H[t,1:numB],gamma[1:numB])
    Z[t] ~ dnorm(mu[t], tau = tau.z)
  }

  beta0 ~ dnorm(0, tau=0.04)
  tau.z ~ dgamma(shape=0.01, rate=0.01)
...
monitors <- c('beta', 'beta0', 'tau.z', 'gamma')
...
###### computation of the full-sample log score for the BP model with basis functions.
lpp <- rep(0, Tobs)
for(j in 1:Tobs){
  
  if(yfull[j]==0){
    temp<-sum(pnorm(0, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                      c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                    sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
  else if(yfull[j]==1){
    temp <- sum(pnorm(1, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                        c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                      sd = 1/sqrt(samp[,"tau.z"]))-
                  pnorm(0, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                          c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                        sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
  else if(yfull[j]==2){
    temp <- sum(pnorm(2, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                        c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                      sd = 1/sqrt(samp[,"tau.z"]))-
                  pnorm(1, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                          c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                        sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
  else if(yfull[j]==3){
    temp <- sum(pnorm(3, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                        c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                      sd = 1/sqrt(samp[,"tau.z"]))-
                  pnorm(2, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                          c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                        sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
  else if(yfull[j]==4){
    temp <- sum(pnorm(4, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                        c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                      sd = 1/sqrt(samp[,"tau.z"]))-
                  pnorm(3, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                          c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                        sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
  else if(yfull[j]==5){
    temp <- sum(1-pnorm(4, mean = samp[,"beta0"]+samp[,"beta[2]"]*X[j,2]+samp[,"beta[3]"]*X[j,3]+
                          c(as.matrix(H[j,1:numB]) %*% t(g.samp[,1:numB])), 
                        sd = 1/sqrt(samp[,"tau.z"])))
    lpp[j] <- log(temp/nrow(samp))
  }
